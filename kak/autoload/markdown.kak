...
add-highlighter shared/markdown/codeblock region -match-capture \
    ...
    ^(\h*)```\h*$ \
    regions
add-highlighter shared/markdown/codeblock/ default-region fill block
add-highlighter shared/markdown/listblock/codeblock region -match-capture \
    ^(\h*)```\h* \
    ^(\h*)```\h*$ \
    regions
add-highlighter shared/markdown/listblock/codeblock/ default-region fill block
add-highlighter shared/markdown/codeline region "^( {4}|\t)" "$" fill block
...
add-highlighter shared/markdown/inline/text/ regex "\H( {2,})$" 1:+r@block
...
define-command markdown-load-languages -params 1 %{
    evaluate-commands -draft %{ try %{
        ...
        evaluate-commands -itersel %{ try %{
            ...
            add-highlighter "shared/markdown/codeblock/%val{selection}/" default-region fill block
            ...
            add-highlighter "shared/markdown/listblock/codeblock/%val{selection}/" default-region fill block
