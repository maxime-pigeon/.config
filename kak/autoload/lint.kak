...
%{
    ...
    evaluate-commands %sh{
        ...
        awk -v file="$kak_buffile" -v stamp="$kak_timestamp" -v client="$kak_client" '
            ...
            /:[1-9][0-9]*:[1-9][0-9]*:/ {
                ...
                if ($4 ~ /[Ee]rror/) {
                    ...
                    flags_by_line[$2] = "{DiagnosticError}x"
                    error_count++
                } else if (flags_by_line[$2] ~ /DiagnosticError/) {
                    ...
                    flags_by_line[$2] = "{DiagnosticWarning}!"
                    warning_count++
                }
        ...
        if [ -s "$dir"/stderr ]; then
            # DiagnosticErrors were detected!"
            ...
            printf "eval -client %s echo -markup {DiagnosticError}%s\n" \
                "$kak_client" \
...
define-command -hidden lint-show-counters %{
    echo -markup "linting results: {DiagnosticError} %opt{lint_error_count} error(s) {DiagnosticWarning} %opt{lint_warning_count} warning(s) "
}
...
%{
    ...
    evaluate-commands %sh{
        ...
        printf "echo -markup \
            {DiagnosticWarning}lint message search wrapped around buffer\n"
